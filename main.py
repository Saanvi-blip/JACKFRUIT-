...
FLASHCARD_FILE = os.path.join(BASE_DIR, "flashcards.json")
...
if page == "🔄 FORMULA Flashcards":
    ...
    if len(st.session_state.flashcards) == 0:
        ...
        # 🔹 Get subjects dynamically (includes newly added subjects)
        subjects = sorted(
            list(set(c["subject"] for c in st.session_state.flashcards))
        )
        ...
        if selected_subject != "All":
            filtered = [
                c for c in filtered if c["subject"] == selected_subject
            ]
        ...
        if len(filtered) == 0:
            ...
            with col2:
                st.markdown(
                    ...
                    f"Card {st.session_state.current_index + 1} of {len(filtered)}"
                    f"</strong></div>",
            ...
            if not st.session_state.is_flipped:
                st.markdown(
                    ...
                    <div style="
                        ...
                        <h2>{card['front']}</h2>
                        <p style="opacity:0.7;">
                            Click 'Answer Key' to see the solution
                        </p>
                    </div>
                    """,
                    unsafe_allow_html=True
...
if page == "🛠️ Create Flashcards":
    st.title("🛠️ Create Your Own Flashcards")

    with st.form("create_flashcard_form"):
        col1, col2 = st.columns(2)

        # 🔹 Get existing subjects dynamically
        existing_subjects = sorted(
            list(set(c["subject"] for c in st.session_state.flashcards))
        )

        subject_options = existing_subjects + ["Other"]
        ...
        if submitted:
            ...
            final_subject = new_subject.strip() if subject == "Other" else subject

            if final_subject and front.strip() and back.strip():
                ...
                st.success(f"Flashcard added under '{final_subject}'!")
                st.rerun()
            else:
                st.error("Please fill all fields and enter a subject name.")
